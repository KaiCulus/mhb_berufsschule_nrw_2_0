<script setup>
/**
 * TicketStatusSelect — Status-Anzeige und -Änderung
 *
 * Im Readonly-Modus: Farbiger Badge mit aktuellem Status.
 * Im Edit-Modus: Dropdown zur Statusänderung.
 *
 * Props:
 *   modelValue — Aktueller Status-Wert (z.B. 'open', 'in_progress')
 *   isReadonly — Ob das Feld editierbar ist
 * Emits:
 *   update:modelValue — Neuer Status-Wert bei Änderung
 */
...
<template>
  <div class="status-select">
    ...
    <div
      ...
      class="status-badge"
      :style="{
        backgroundColor: statuses.find(s => s.value === modelValue)?.color + '20',
        color: statuses.find(s => s.value === modelValue)?.color
      }"
    >
      {{ getLabel(modelValue) }}
    </div>
