 ...
 *
 * Zeigt alle Details eines Tickets und erlaubt berechtigten Usern das
 * direkte Bearbeiten von Feldern (jedes Feld sendet beim Ändern sofort
 * ein update-field-Request ans Backend).
 *
 * Editierbarkeit wird server-seitig via `ticket.can_edit_status` gesteuert
 * und an alle Unterkomponenten als :is-readonly weitergegeben.
 *
 * Lösch-Berechtigung wird separat via canDeleteTicket-Endpoint geprüft:
 *   - canDelete = true  → User kann sein eigenes Ticket zurückziehen (löschen)
 *   - can_edit_status   → Bearbeiter können das Ticket als erledigt markieren
 *
 * Emits:
 *   close   — Schließt das Modal
 *   refresh — Signalisiert der Ticket-Liste, sich neu zu laden
 *
 * Props:
 *   ticketId — ID des anzuzeigenden Tickets
 */
...
const fetchDetails = async () => {
  try {
    ...
    console.error('Ticket-Details konnten nicht geladen werden:', error);
  }
};
...
onMounted(async () => {
  loading.value = true;
  // Reihenfolge wichtig: Details müssen vor Berechtigungsprüfung geladen sein
  await fetchDetails();
  await checkDeletePermission();
  loading.value = false;
});

/** Sendet eine einzelne Feldänderung ans Backend und aktualisiert danach die Ansicht. */
...
/** Markiert das Ticket als erledigt oder zieht es zurück (je nach Berechtigung). */
...
/** Löscht alle erledigten Tickets älter als 7 Tage (Admin-Funktion). */
  ...
  <div class="modal-overlay" @click.self="emit('close')">
    <div class="modal-content" v-if="ticket">
      ...
      <div class="modal-body">
        <div class="details-grid">
          <section class="info-section">
            ...
            <!-- Raum-Abo-Toggle direkt unter dem Ort anzeigen wenn ein Raum bekannt ist -->
      ...
      <footer class="modal-footer">
        <!-- Bereinigung: nur für Bearbeiter sichtbar, nicht wenn User sein eigenes Ticket löschen kann -->
        ...
        <div class="main-footer-actions">
          <button v-if="canDelete" @click="resolveTicket" class="delete-btn">
            🗑️ Ticket zurückziehen
          </button>
          <button v-else-if="ticket.can_edit_status" @click="resolveTicket" class="resolve-btn">
