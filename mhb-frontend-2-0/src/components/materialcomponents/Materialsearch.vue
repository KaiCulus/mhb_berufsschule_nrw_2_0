...
/**
 * Materialsearch — Echtzeit-Suche mit Debounce
 *
 * Sucht ab 2 Zeichen Eingabe mit 300ms Debounce gegen den Backend-Endpoint.
 * Zeigt Ergebnisse als Cards an.
 *
 * Bearbeiten und Löschen sind nur für den Ersteller eines Eintrags sichtbar.
 * Nach dem Löschen wird der Eintrag sofort aus der lokalen Ergebnisliste
 * entfernt — kein erneuter API-Call nötig.
 *
 * Emits:
 *   edit    — Gibt das Material-Objekt an die Elternseite weiter
 *   deleted — Signalisiert der Elternseite, dass ein Eintrag gelöscht wurde
 *             (damit Materialliste sich refreshen kann)
 */
...
const deleteItem = async (id) => {
  ...
  try {
    ...
    // Optimistisch aus der lokalen Liste entfernen ohne erneuten API-Call
...
// 300ms Debounce: verhindert einen API-Call bei jedem Tastendruck
let timeout = null;
watch(searchQuery, () => {
